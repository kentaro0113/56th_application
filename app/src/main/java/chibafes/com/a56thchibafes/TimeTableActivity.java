...
public class TimeTableActivity extends Fragment {
    ...
    private void setList(int nextList) {
        ...
        if(nextList == BUTTON_TIMETABLE_TAB_CHECK) {
            if (arrayCheck != null) {
                // 指定日のステージのリスト生成
                for (int h = 0; h < arrayCheck.length; ++h) {
                    int nTargetIndex = arrayCheck[h] / 10000;
                    int nTargetTime = arrayCheck[h] % 10000;
        ...
        if(nCurrentList == BUTTON_TIMETABLE_TAB_CHECK) {
            ...
            if(arrayCheck != null) {
                for(int i = 0; i < arrayCheck.length; ++i) {
                    int nTargetIndex = arrayCheck[i] / 10000;
                    if(itemTimeTable.getIndex() != nTargetIndex) {
                        continue;
                    }
                    int nTargetTime = arrayCheck[i] % 10000;
    ...
    class TimeTableItem {
        ...
        public boolean setData(Dict data, int nIndex, KikakuItem[] arrayKikakuData) {
            try {
                this.nIndex = nIndex;
                nId = data.getConfigurationInteger("id").getValue();
                nTime = data.getConfigurationInteger("time").getValue();

                for(int i = 0; i < arrayKikakuData.length; ++i) {
                    int n = arrayKikakuData[i].getIntValue("id");
                    if(arrayKikakuData[i].getIntValue("id") == getId()) {
                        itemKikaku = arrayKikakuData[i];
                        break;
                    }
                }
    ...
    class TimeTableListAdapter extends ArrayAdapter<TimeTableItem> {
        ...
        public TimeTableListAdapter(Context context, int textViewResourceId, List<TimeTableItem> objects) {
